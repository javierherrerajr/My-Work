...
int main()
{
    ...
    object* object_ptrs_balls[10];
    object* object_ptrs_points[10];

    for (size_t i = 0; i < 10; i++)
    {
        object_ptrs_balls[i] = &balls[i];  // Pointers to base class
        object_ptrs_points[i] = &points[i];
    }

    set_names(object_ptrs_balls, 10, "ball-");
    set_names(object_ptrs_points, 10, "point-");
