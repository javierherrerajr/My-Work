...
get_string_loop ; loops until ENTER is inputted
    ...
    out
    add r3, r0, #-10
    brn backspace_check ; checks for any backspace characters
...
backspace_check
    and r0, r0, #0 ; sets r0=0 for later storing
    add r1, r1, #-1 ; goes back to previous character stored
    str r0, r1, #0
    add r5, r5, #-1
    brnzp get_string_loop ; r0 is now 0 and will store in previous space
...
ld r4, lower_conversion

lower_to_upper
    ldr r0, r1, #0 ; gets value of character in array
    brnzp lower_checker ; checks if letter is already capital
    
    lower_to_upper_convert
        and r2, r2, #0
        add r2, r0, r4
        and r3, r2, r0 ; converts from lower to upper
        str r3, r1, #0 ; stores the converted letter into the array
...
restore_jumper
    add r0, r0, #0
    brnzp restore_registers
    
lower_checker
    ld r2, lower_checker_diff
    add r2, r0, r2
    brn next_character
    brzp lower_to_upper_convert
...
ret
;data
lower_conversion    .fill   #-32
lower_checker_diff  .fill   #-97
.end
